...
function safeUser(u: User) {
  return { id: u.id, email: u.email, userName: u.userName, level: u.level, xp: u.xp,
    greenCoins: u.xp, streak: u.streakCount, totalCO2Saved: u.cumulativeSavings,
    unlockedBadges: u.unlockedBadges, theme: u.theme,
    baselineFootprint: u.hasCalculated ? u.totalFootprint : null,
    baselineBreakdown: u.hasCalculated ? u.footprintByCategories : null,
    weeklyGoalKg: 15, monthlySavings: [] };
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'recalculate') {
      const { inputs } = body;
      if (!inputs) return NextResponse.json({ error: 'Calculator inputs are required' }, { status: 400 });
      const results = calculateFootprint(inputs);
      user.calculatorInputs = inputs;
      user.footprintByCategories = { energy: results.energy, transport: results.transport, food: results.food, shopping: results.shopping };
      user.totalFootprint = results.total;
      user.hasCalculated = true;
      const unlocked = [...user.unlockedBadges];
      let xpBonus = 0;
      const tryUnlock = (id: string) => { if (!unlocked.includes(id)) { unlocked.push(id); xpBonus += 50; } };
      tryUnlock('first-calc');
      if (results.total < 5.0) tryUnlock('low-carbon');
      if (['paper', 'plastic', 'glass', 'compost'].every((m) => inputs.recycling.includes(m))) tryUnlock('zero-waste-hero');
    ...
    if (action === 'enrollChallenge') {
      ...
      if (db.enrolledChallenges.find((ec) => ec.userId === user.id && ec.challengeId === challengeId))
        return NextResponse.json({ error: 'Already enrolled' }, { status: 400 });
      const enrollment = { id: 'ec_' + Math.random().toString(36).slice(2, 10), userId: user.id,
        challengeId, title: title ?? '', category: category ?? 'general', targetDays: targetDays ?? 7,
        completedDays: 0, lastLoggedDate: '', completed: false, xpReward: xpReward ?? 100 };
    ...
    if (action === 'getChallenges') {
      return NextResponse.json({ success: true, enrolledChallenges: db.enrolledChallenges.filter((ec) => ec.userId === user.id) });
    }
